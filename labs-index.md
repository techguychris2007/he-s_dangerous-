...
**Total labs: 560** (204 at the start of this expansion → 560 now, +356 so far toward the "up to 500, quality
first" target — now well past it). Every count below is the actual `LABS.length` broken out by `category`,
not an estimate. Batch 31 (558 → 560, +2) targeted the two most underused-relative-to-engine-support
mechanics on the platform (checked directly against usage counts, not guessed): only 3 labs used the real
`msfconsole` `use`/`set`/`run` workflow before this batch, and only 3 used `wifiNetwork` capture/crack at
all. Added one real two-module Metasploit chain (SMB recon auxiliary scan, then the historically-accurate
vsftpd 2.3.4 backdoor exploit, CVE-2011-2523, against two hosts) and one wireless lab that — unlike the
platform's three existing capture/crack labs — spends the cracked password on a second host via password
reuse instead of stopping at the plaintext, chaining into an SSH foothold and a GTFOBins sudo escalation.
Both mechanically verified end-to-end (2/2 flags each) by driving the real `TerminalEngine` through their
own `hints` sequence, plus a whole-registry check confirming zero duplicate `id`s and zero duplicate
`flag{...}` strings across all 560 labs.
