...
export default function CodeEditor({ value, onChange, disabled }: CodeEditorProps) {
  ...
  return (
    <textarea
      ...
      autoCapitalize="off"
      autoCorrect="off"
      className="w-full h-full min-h-[280px] resize-none bg-[var(--term-bg)] text-[var(--term-output)] font-mono text-code leading-relaxed p-4 outline-none rounded-lg border border-[var(--color-accent)]/30 focus:border-[var(--color-accent)]/60 disabled:opacity-60"
      style={{ tabSize: 4 }}
    />
  );
}
