...
import { ModuleIcon, IconDashboard, IconFlask, IconChart, IconMap, IconCheck } from './icons';
  ...
  return (
    <aside className="w-72 shrink-0 h-full overflow-y-auto border-r border-[var(--color-border)] bg-[var(--color-surface)] px-4 py-5 flex flex-col">
      ...
      <div className="px-2 mb-5">
        <div className="text-[10px] font-mono font-bold uppercase tracking-widest text-[var(--color-accent)]">Learner Portal</div>
        <div className="text-xs font-semibold text-[var(--color-heading)] uppercase tracking-wide truncate">
          {currentModule ? currentModule.title : 'Offensive Security'}
        </div>
      </div>
      ...
      <div className="border-t border-[var(--color-border)] pt-3 mt-3 flex items-center gap-2.5 px-2">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="text-[11px] text-[var(--color-text-dim)]">Offensive Security</div>
        </div>
        <button
          onClick={() => {
            progress.logout();
