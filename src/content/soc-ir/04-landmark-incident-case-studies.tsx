...
-> a single misspelled word triggered a manual compliance review,
   which froze that transfer before it could be laundered.`}</CodeBlock>
      ...
      <h2>What both incidents actually have in common</h2>
      <p>
        Neither breach succeeded because detection was impossible. Target's system correctly identified the
        malware; Bangladesh Bank's automated filters correctly blocked most of the fraud. Both incidents
        instead turned on what happens in the gap between a system producing a correct signal and a human (or
        an automated response) acting on it in time — the exact gap SOAR automation, covered in the previous
        lesson, exists to close for the cases that can be safely automated, and disciplined triage exists to
        close for the cases that still need a human judgment call.
      </p>
      ...
      <p>
        ...
        automation, reporting, and — as these two case studies show — the very human factors that determine
        whether a correct detection actually stops anything. The SOC Portal's SIEM labs let you practice every
        one of these mechanics directly, inside real platform-style consoles, against both realistic
        essentials scenarios and reconstructions of major real-world breaches.
      </p>
    </div>
  );
}
